...
const httpLogger = pinoHttp({
  ...
  serializers: {
    ...
    res: (res) => ({
      ...
      responseBody: (res as any).raw?.responseBody,
    }),
  },
});

function captureResponseBody(_req: Request, res: Response, next: NextFunction): void {
...
async function bootstrapSweep(): Promise<void> {
  ...
  if (result.count > 0) {
    bootstrapLogger.info({ recovered: result.count }, 'Reset stuck RUNNING jobs to PENDING');
  }
}
