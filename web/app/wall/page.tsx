...
export default function WallPage() {
  ...
  // ── Shared: Featured match center content ──
  const renderFeaturedMatch = (compact = false) => {
    if (!selectedMatch) return null;
    const live     = isLive(selectedMatch);
    const finished = isFinished(selectedMatch);
    const predict  = canPredict(selectedMatch);
    const flagSize = compact ? 32 : 52;

    return (
      <>
        {!compact && (
          <div className="resp-hide-mobile" style={{ position: 'absolute', fontSize: 280, fontWeight: 900, color: 'rgba(255,215,0,0.025)', letterSpacing: -20, userSelect: 'none', pointerEvents: 'none', lineHeight: 1, zIndex: 0 }}>
            {(live || finished)
        ...
        <div style={{ width: '100%', maxWidth: 560, background: 'var(--bg2)', border: `1px solid ${live ? 'rgba(255,64,64,0.4)' : 'var(--b2)'}`, borderRadius: 10, padding: compact ? '16px 18px' : '22px 26px', position: 'relative', zIndex: 2 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
            <div style={{ textAlign: 'center', flex: 1 }}>
              <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 8 }}>
                <FlagImg url={selectedMatch.homeFlagUrl} alt={selectedMatch.homeTeamName} size={flagSize} />
              </div>
              <div className="wall-team-name" style={{ fontSize: 22, fontWeight: 900, letterSpacing: 3 }}>{selectedMatch.homeTeamName.toUpperCase()}</div>
            </div>
              ...
              {(live || finished) && selectedMatch.homeScore !== null ? (
                <>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    <span className="wall-score-display" style={{ fontSize: compact ? 48 : 80, fontWeight: 900, color: live ? 'var(--red)' : 'var(--t1)', letterSpacing: -4, lineHeight: 1 }}>{selectedMatch.homeScore}</span>
                    <span style={{ fontSize: compact ? 24 : 40, fontWeight: 300, color: 'var(--t3)', margin: '0 4px' }}>—</span>
                    <span className="wall-score-display" style={{ fontSize: compact ? 48 : 80, fontWeight: 900, color: live ? 'var(--red)' : 'var(--t1)', letterSpacing: -4, lineHeight: 1 }}>{selectedMatch.awayScore}</span>
                  </div>
            ...
            <div style={{ textAlign: 'center', flex: 1 }}>
              <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 8 }}>
                <FlagImg url={selectedMatch.awayFlagUrl} alt={selectedMatch.awayTeamName} size={flagSize} />
              </div>
              <div className="wall-team-name" style={{ fontSize: 22, fontWeight: 900, letterSpacing: 3 }}>{selectedMatch.awayTeamName.toUpperCase()}</div>
            </div>
          </div>
  ...
  if (isMobile) {
    return (
      <div style={{ width: '100vw', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        {/* Compact header */}
        <div style={{ height: 56, flexShrink: 0, background: 'var(--header-bg)', borderBottom: '1px solid var(--b2)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px', position: 'sticky', top: 0, zIndex: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <Image src="/ibplc-logo.png" alt="IBPLC" width={28} height={38} style={{ display: 'block' }} />
            <div className="title-lg" style={{ fontWeight: 900, color: 'var(--c)', lineHeight: 1 }}>BATTLE OF THE BRANDS</div>
          </div>
          {liveFixtures.length > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(255,64,64,0.12)', border: '1px solid rgba(255,64,64,0.4)', padding: '5px 10px', borderRadius: 4 }}>
              <div style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--red)', animation: 'pulse-dot 1.2s infinite' }} />
              <span style={{ fontSize: 11, fontWeight: 800, letterSpacing: 2, color: '#ff7070' }}>LIVE</span>
            </div>
          )}
        </div>

        {/* Scrollable content */}
        <div style={{ flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch' as any, position: 'relative', zIndex: 5 }}>

          {/* Featured match */}
          <div style={{ padding: '20px 16px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0 }}>
            {renderFeaturedMatch(true)}
          </div>
          ...
          {/* CTA strip */}
          <div className="wall-cta-strip" style={{ background: 'rgba(255,215,0,0.06)', borderTop: '1px solid rgba(255,215,0,0.2)', padding: '12px 16px', display: 'flex', alignItems: 'flex-start', gap: 12 }}>
            <div style={{ fontSize: 14, fontWeight: 800, letterSpacing: 2 }}>
              SCAN TO PREDICT <span style={{ color: 'var(--c)' }}>·</span> BEAT THE BOTB
            </div>
            <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' as any }}>
              {[['50','EXACT SCORE'],['20','CORRECT WINNER'],['30','UPSET BONUS'],['10','FIRST SCORER']].map(([val,label]) => (
                <div key={label} style={{ textAlign: 'center' }}>
                  <span style={{ display: 'block', fontSize: 16, fontWeight: 900, color: 'var(--gold)', lineHeight: 1 }}>{val}</span>
          ...
          {/* Ticker */}
          <div style={{ height: 40, background: 'var(--bg2)', borderTop: '1px solid var(--b1)', display: 'flex', alignItems: 'center', overflow: 'hidden' }}>
